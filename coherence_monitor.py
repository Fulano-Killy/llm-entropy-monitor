...
class EntropyMonitor:
    ...
    """
    def __init__(self, entropy_threshold=2.5, window_size=5):
...
if __name__ == "__main__":
    # Simulating a looping state
    monitor = EntropyMonitor()
    test_stream = ["data", "process", "error", "error", "error", "error", "error"]
    
    print("--- System Diagnostic ---")
    for token in test_stream:
        result = monitor.analyze_stream(token)
        if result["status"] == "CRITICAL":
            print(f"ALARM TRIGGERED: {result}")
            break
        else:
            print(f"Processing: {token} (Stable)")
